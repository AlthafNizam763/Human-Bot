...
exports.handleGetPersonalities = handleGetPersonalities;
exports.handleCreatePersonality = handleCreatePersonality;
exports.handleUpdatePersonality = handleUpdatePersonality;
exports.handleDeletePersonality = handleDeletePersonality;
const firebase_js_1 = require("../config/firebase.js");
const firebase_admin_1 = __importDefault(require("firebase-admin"));
/**
 * Get all personality profiles
 */
async function handleGetPersonalities(req, res) {
 ...
 */
async function handleCreatePersonality(req, res) {
    const userId = req.user?.uid;
 ...
 */
async function handleUpdatePersonality(req, res) {
    const userId = req.user?.uid;
    const { id } = req.params;
    const { name, description, prompt } = req.body;
    if (!userId || !id) {
    ...
    try {
        const docRef = firebase_js_1.db.collection('personalities').doc(id);
 ...
 */
async function handleDeletePersonality(req, res) {
    const userId = req.user?.uid;
    const { id } = req.params;
    if (!userId || !id) {
        return res.status(400).json({ error: 'Missing personality ID or user context.' });
